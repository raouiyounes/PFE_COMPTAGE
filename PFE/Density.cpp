...
Mat thresh=Mat::zeros(img.size(),CV_8UC1);
  ...
  // les annotations sont stockees dans le fichier annotation.txt 
ofstream annotation("annotation.txt");
vector<int>  anot_xx;
vector<int>  anot_yx;
...
Density::Density() {
  ...
  // avoir les coordonnes des points qui sont cliques par la sourie
  
void Density::getXY(){
	Mat img = cv::imread("image_test.jpeg"); // force grayscale
	Mat thresh=Mat::zeros(img.size(),CV_8UC1);
...
MatrixXd annot(2,2);
// les annotations sont stockees dans les vecteurs anot_x_coor et anot_t_coor
...
Density::~Density() {
  ...
  //calcul de F0 F^{i}_{0}(p)=\sum_{P \in P_{i}} N(p;P,\sigma^{2} 1_{2*2})  
...
int anotate;
MatrixXd patch(3,3);
Mat imgMat=imread("image_test.jpeg");
float pixelValue;
...
F0_mat.push_back(f002/5);
 ...
 // retourner la valeur de F0_mat
return F0_mat;
}
  // fonction pour le clique de la sourie
...
{                    
  ...
  //lecture des annotations du fichier annotation.txt 
  
MatrixXd Density::readAnnotations(){
	int numberof_annot;
	MatrixXd annot_loc(numberof_annot,2);
	ifstream myfile ("annotation.txt");
	stringstream ss;
	string s;
	int j=0;
	string line;

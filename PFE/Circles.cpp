...
public:
  ...
  // extraction des points d'interet SURF
  
int SURFExtractor(int nb_of_features){
namedWindow( "image", CV_WINDOW_AUTOSIZE );
cvShowImage( "image",  image_matrix );
  cvWaitKey(0);
...
int dim=50;
 while(j<nb_of_features){
	...
	//les lignes du descripteurs sont inserees dans descriptor_vect
  ...
  // Extraction des points d'interet BRISC
  ...
  int extractBrisc(char *str)
  {
    ...
    IplImage* image=cvLoadImage(str);
      int i;
      CvMat *image1=0;
      // les coordonnees et les descripteurs sont stockes dans le fichier brisc_db.txt
         ofstream outputFile("brisc_db.txt");
